...
This section describes the physical/virtual infrastructure required to run the Copilot agents.
...
│                                                                                                     │
│  ┌─────────────────────────────────────────┐       ┌─────────────────────────────────────────────┐ │
│  │     🖥️ GITHUB ENTERPRISE SERVER (GHES)  │       │        🏃 SELF-HOSTED RUNNER VM             │ │
│  │              (Primary VM)               │       │           (Actions Executor)                │ │
│  │                                         │       │                                             │ │
│  │  ┌─────────────────────────────────┐   │       │  ┌─────────────────────────────────────┐   │ │
│  │  │ 📦 GitHub Enterprise Server     │   │       │  │ 🔧 Pre-installed Software           │   │ │
│  │  │    • Web UI & API               │   │       │  │    • GitHub Actions Runner Agent   │   │ │
│  │  │    • Git Repositories           │   │◀─────▶│  │    • GitHub CLI (gh) ⚠️ REQUIRED   │   │ │
│  │  │    • Issues & Pull Requests     │   │  API  │  │    • git, curl, jq                 │   │ │
│  │  │    • Actions Orchestration      │   │       │  └─────────────────────────────────────┘   │ │
│  │  └─────────────────────────────────┘   │       │                                             │ │
│  │                                         │       │  ┌─────────────────────────────────────┐   │ │
│  │  ┌─────────────────────────────────┐   │       │  │ ⚡ Installed by Workflow (Runtime)   │   │ │
│  │  │ 🔐 Secrets Storage              │   │       │  │    • Node.js 22.x                   │   │ │
│  │  │    • GH_TOKEN                   │───────────▶  │    • Python 3.x + uv/uvx            │   │ │
│  │  │    • COPILOT_TOKEN              │   │       │  │    • GitHub Copilot CLI             │   │ │
│  │  │    • CONTEXT7_API_KEY           │   │       │  │    • MCP Servers (Context7, Fetch)  │   │ │
│  │  └─────────────────────────────────┘   │       │  └─────────────────────────────────────┘   │ │
│  │                                         │       │                                             │ │
│  │  ┌─────────────────────────────────┐   │       │  ┌─────────────────────────────────────┐   │ │
│  │  │ 📂 Repositories                  │   │       │  │ 🤖 Copilot Agents (Runtime)         │   │ │
│  │  │    • GHES_CodingAgent (central) │   │       │  │                                     │   │ │
│  │  │    • Target repos (with caller  │   │       │  │  ┌─────────────┐ ┌─────────────┐   │   │ │
│  │  │      workflows deployed)        │   │       │  │  │ Coder Agent │ │Review Agent │   │   │ │
│  │  └─────────────────────────────────┘   │       │  │  │             │ │             │   │   │ │
│  │                                         │       │  │  │ • Reads     │ │ • Downloads │   │   │ │
│  │  ┌─────────────────────────────────┐   │       │  │  │   issue     │ │   PR files  │   │   │ │
│  │  │ 🏷️ Labels                        │   │       │  │  │ • Generates │ │ • Analyzes  │   │   │ │
│  │  │    • copilot (trigger)          │   │       │  │  │   code      │ │   code      │   │   │ │
│  │  │    • in-progress               │   │       │  │  │ • Creates   │ │ • Posts     │   │   │ │
│  │  │    • ready-for-review          │   │       │  │  │   PR        │ │   comments  │   │   │ │
│  │  └─────────────────────────────────┘   │       │  │  └─────────────┘ └─────────────┘   │   │ │
│  │                                         │       │  └─────────────────────────────────────┘   │ │
│  └─────────────────────────────────────────┘       └─────────────────────────────────────────────┘ │
│           │                                                         │                              │
...
│                                                                                                      │
│   ┌─────────────────┐   ┌─────────────────┐   ┌─────────────────┐   ┌─────────────────────────────┐ │
│   │ 📦 npm Registry │   │ 🐍 PyPI         │   │ 🤖 Copilot API  │   │ 📚 Context7 (Optional)      │ │
│   │                 │   │                 │   │                 │   │                             │ │
│   │ registry.       │   │ pypi.org        │   │ copilot-api.    │   │ api.context7.com            │ │
│   │ npmjs.org       │   │ files.python    │   │ github.com      │   │                             │ │
│   │                 │   │ hosted.org      │   │                 │   │ (Documentation service)     │ │
│   │ Downloads:      │   │                 │   │ Provides:       │   │                             │ │
│   │ • Copilot CLI   │   │ Downloads:      │   │ • AI code       │   │ Provides:                   │ │
│   │ • Context7 MCP  │   │ • mcp-server-   │   │   generation    │   │ • Library docs              │ │
│   │                 │   │   fetch         │   │ • Code review   │   │ • Best practices            │ │
│   │                 │   │ • mcp-server-   │   │   analysis      │   │ • Code examples             │ │
│   │                 │   │   time          │   │                 │   │                             │ │
│   └─────────────────┘   └─────────────────┘   └─────────────────┘   └─────────────────────────────┘ │
│                                                                                                      │
└──────────────────────────────────────────────────────────────────────────────────────────────────────┘
...
#### 🖥️ GitHub Enterprise Server (GHES) VM

The primary server running your GitHub Enterprise instance.

| Component | Description |
|-----------|-------------|
| **Web UI & API** | User interface and REST/GraphQL APIs for repository operations |
| **Git Repositories** | Stores `GHES_CodingAgent` (central) and all target repositories |
| **Issues & PRs** | Where users create issues and Copilot creates pull requests |
| **Actions Orchestration** | Triggers and manages workflow execution on runners |
| **Secrets Storage** | Securely stores `GH_TOKEN`, `COPILOT_TOKEN`, etc. |
| **Labels** | `copilot`, `in-progress`, `ready-for-review` for workflow control |

**Typical Specifications:**

| Spec | Recommendation |
|------|----------------|
| **vCPUs** | 8-16+ (depends on org size) |
| **RAM** | 32-64+ GB |
| **Storage** | 500+ GB SSD |
| **OS** | Per GitHub requirements |

#### 🏃 Self-Hosted Runner VM

A dedicated VM that executes GitHub Actions workflows.

| Component | Category | Description |
|-----------|----------|-------------|
| **Actions Runner Agent** | Pre-installed | GitHub's runner software that picks up jobs |
| **GitHub CLI (`gh`)** | Pre-installed ⚠️ | **MUST be installed manually** - used for API calls |
| **git, curl, jq** | Pre-installed | Standard tools for git operations and API calls |
| **Node.js 22.x** | Workflow-installed | Runtime for Copilot CLI and Context7 MCP |
| **Python 3.x + uv** | Workflow-installed | Runtime for Fetch and Time MCP servers |
| **Copilot CLI** | Workflow-installed | The AI engine that generates/reviews code |
| **MCP Servers** | Workflow-installed | Context7, Fetch, Time - extend Copilot capabilities |
...
|-----------|------|-----|------|---------|
| Internal | Runner | GHES | 443 | Git clone/push, API calls |
| Internal | GHES | Runner | Dynamic | Job dispatch |
| Outbound | Runner | registry.npmjs.org | 443 | Download Copilot CLI |
| Outbound | Runner | pypi.org | 443 | Download MCP servers |
| Outbound | Runner | copilot-api.github.com | 443 | AI code generation |
| Outbound | Runner | api.context7.com | 443 | Documentation (optional) |

### Deployment Topology Options

#### Option A: Single VM (Dev/Small Teams)

```
┌─────────────────────────────────────────┐
│           Single VM                     │
│  ┌─────────────────────────────────┐   │
│  │  GHES + Runner (co-located)     │   │
...
#### Option C: Multiple Runners (Enterprise/High Volume)
...
│  │                    APPLICATION LAYER                                 │   │
│  │  ┌──────────────────────┐  ┌──────────────────────┐                 │   │
│  │  │  🤖 Copilot Coder    │  │  🔍 Copilot Reviewer │                 │   │
│  │  │  Agent               │  │  Agent               │                 │   │
│  │  └──────────────────────┘  └──────────────────────┘                 │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
...
│  │                    AI & TOOLS LAYER                                  │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐              │   │
│  │  │ Copilot CLI  │  │ Context7 MCP │  │ Fetch MCP    │              │   │
│  │  │ (@github/    │  │ (npx)        │  │ (uvx)        │              │   │
│  │  │  copilot)    │  │              │  │              │              │   │
│  │  └──────────────┘  └──────────────┘  └──────────────┘              │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
...
│  │                    RUNTIME LAYER                                     │   │
│  │  ┌──────────────────────────────┐  ┌──────────────────────────────┐ │   │
│  │  │  Node.js 22.x                │  │  Python 3.x + uv/uvx         │ │   │
│  │  │  (Copilot CLI, Context7)     │  │  (Fetch, Time MCP servers)   │ │   │
│  │  └──────────────────────────────┘  └──────────────────────────────┘ │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
